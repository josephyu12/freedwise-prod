...
async function replayOne(supabase: any, action: any, userId: string, freq: number): Promise<boolean> {
  switch (action.type) {
    case 'rate-review': {
      ...
      // Average / auto-archive is part of the same rating action — a flaky
      // stats write must keep this item queued (one pending "Rating", not a
      // second stats change). Rating + ledger upserts are idempotent on retry.
    ...
    case 'rate-daily': {
      ...
      // Shared stats + auto-archive is part of this same rating action (see
      // rate-review). Legacy actions queued without summaryDate fall back to
      // the local today.
      const n = new Date()
