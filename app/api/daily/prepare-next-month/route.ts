...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // to scheduled cron invocations when CRON_SECRET is set. Require exactly that
    // token — the old x-vercel-cron / user-agent checks were spoofable, and the
    // service-role-key fallback let the all-users job run unauthenticated in dev.
    // `?secret=` is kept so the job can still be triggered manually with curl.
    const cronSecret = process.env.CRON_SECRET
    if (!cronSecret) {
      console.error('[PREPARE-NEXT-MONTH] CRON_SECRET is not set — refusing to run.')
