...
export default function HighlightsPage() {
  ...
  const handleBulkSetArchived = async (archive: boolean) => {
    const ids = Array.from(selectedIds)
    if (ids.length === 0 || bulkWorking) return
    const selectedItems = ids.map(
      (id) => highlights.find((h) => h.id === id) ?? { archived: showArchived }
    )
    const mismatch = archiveActionError(archive ? 'archive' : 'unarchive', selectedItems)
    if (mismatch) {
      alert(mismatch)
      return
    }
    ...
    <main className="min-h-screen" style={{ background: 'var(--background)' }}>
      ...
      {selectMode && (
        <SelectModeBar
          ...
          onArchive={() => handleBulkSetArchived(true)}
          onUnarchive={() => handleBulkSetArchived(false)}
